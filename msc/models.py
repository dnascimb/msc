...
class Ticket(Base):
    ...
    id = Column(String(36), primary_key=True)
    reporter = Column(String(36), ForeignKey(User.id))
    provider = Column(String(36), ForeignKey(Provider.id))
    type = Column(Integer, ForeignKey(TicketType.id))
    status = Column(Integer, ForeignKey(TicketStatus.id), nullable=False, default=1)
